...
if (!isset($_SESSION["logged"]) && $_SESSION["logged"] != true) {
  header("location: http://talkfriend.atwebpages.com/");
}
...
if (isset($_POST['group-create-submit'])) {
  ...
  if ($num2 > 0) {
    while ($row2 = mysqli_fetch_assoc($result2)) {
      if (password_verify($group_pass, $row2['group-pass'])) {
        $showError = "You can't create a group using this name nor this password, Try to keep something unique";
      }
    }
  } else {
...
if (isset($_POST["delete-group-confirm"])) {
  ...
  if ($result) {
    ...
    $_SESSION["group-name"] = NULL;
    $_SESSION["group-owner"] = false;
    header("location: http://talkfriend.atwebpages.com/group");
  }
}
?>
<!doctype html>
 ...
 <div id="message-container" class="cotainer shadow py-4 my-3 rounded-2 mx-3 px-2 border border-2">
  ...
  <script>
    ...
    $(document).ready(function() {
        ...
        $('#send-message-box').load('/server/server3.php')
