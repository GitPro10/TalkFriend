...
$sql = "SELECT `name`,`message`, EXTRACT( DAY FROM `date`) as 'date', EXTRACT( MONTH FROM `date`) as 'month', EXTRACT( YEAR FROM `date`) as 'year', EXTRACT( HOUR FROM `date`) as 'hour', EXTRACT( MINUTE FROM `date`) as 'minute' FROM `messageDB`;";
...
$options = 0;
  ...
  if ($_SESSION["name"] != $name && $name != NULL) {
    echo('
